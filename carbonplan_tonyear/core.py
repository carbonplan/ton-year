...
def calculate_tonyears(method, baseline, time_horizon, delay, discount_rate):
    """This function calculates the benefit of a delayed emission according one of three ton-year accounting methods. 
    
    Keyword arguments: 
        method – string specifying the ton-year accounting method (Moura-Costa: \'mc\', Lashof: \'lashof\', or IPCC \'ipcc\')
        baseline – array modeling the residence of an emission in the atmosphere over time, i.e. a decay curve / impulse response function
        time_horizon – integer specifying the period over which the impact of an emission is considered (years)
        delay – integer specifying the emission delay for which a ton-year benefit will be calculated (years)
        discount_rate – float specifying the discount rate to apply time preference to both costs and benefits over the time horizon
    ...
    Function returns a dictionary with: 
        ...
        benefit – the benefit of delaying an emission, calculated according to specified accounting method (ton-years)
        num_for_equivalence – the ratio between the baseline cost and the benefit from the specified delay 
    """
