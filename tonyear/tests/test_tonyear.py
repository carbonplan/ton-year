...
def test_get_baseline_curve_raises_invalid_args() -> None:
    ...
    with pytest.raises(
        ValueError, match="No baseline curve parameters by the name foo."
    ):
...
def test_calculate_tonyears(
    curve_name, method, time_horizon, delay, discount_rate
) -> None:
...
def test_calculate_tonyears_raises_invalid_args() -> None:
    ...
    with pytest.raises(
        ValueError,
        match="Time horizon cannot be longer than length of the baseline array.",
    ):
